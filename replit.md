...
-   **Client CRM**: Full BridgeX CRM-inspired design clone with 20+ pages. Dashboard features 12 stat cards in 3 rows (Wallet, Deposits, Withdrawals, Commissions, Trading Accounts, Prop Accounts, Investments, Support, IB/Affiliate/Referral Earnings, Loyalty Points) plus Recharts area/bar charts. Wallet page has gradient hero, payment methods grid (including wallet-to-wallet P2P transfer), balance chart. Forex section includes dashboard with KPIs + Profit Growth/Volume Traded charts + Live Open Positions table (with Account ID & Platform columns), trading accounts with BridgeX dark card design (always dark bg, 2x2 stats grid, 7-action icon bar: View/Password/Leverage/Deposit/Withdraw/Transfer/Bonus), account overview page matching BridgeX (4 KPIs + 2 charts + positions table + deposit/withdraw/transfer dialogs), multi-step account creation wizard, finance page as trading operations hub (Overview/Deposit/Withdraw/Internal Transfer tabs, commission transfer dialog with partial amount support), Password change with Master/Investor toggle, Withdraw dialog with multi-currency wallet conversion (USD/EUR/GBP). Additional pages: Offers, IB dashboard with referral tree, PAMM managers, Copy Trading providers, Prop Trading (challenge cards), Investment (portfolio pie chart, plans), Loyalty Points (tier progress, rewards), P2P Exchange (buy/sell), AI Center (chat interface), Download Platform (5 platform cards), Widgets, Profile (gradient header), KYC (verification steps), Settings, Support tickets.
